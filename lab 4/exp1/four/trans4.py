...
# Exp 1 Data
Exp1_Ib = open('four/Ib.txt', 'r').read().split()
Exp1_Ie = open('four/Ie.txt', 'r').read().split()
Exp1_Vb = open('four/Vb.txt', 'r').read().split()
linear_v = []
linear_i = []
beta_vals = []
...
for x in Exp1_Ib:
    ...
    I_cVals4.append((-1*I_eval) - I_bval)
    Exp1_Ib[i] = I_bval
...
U_t4 = (1/(slope))
I_s4 = np.exp(intercept)

i = 0
for x in Exp1_Ib:
    x = float(x)
    beta4 = (I_s4/x)*np.exp(V_bVals[i]/U_t4)
    beta_vals.append(beta4)
    i+=1

beta4 = np.mean(beta_vals)
beta4 = beta_vals
