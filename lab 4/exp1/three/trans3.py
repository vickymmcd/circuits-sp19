...
# Exp 1 Data
Exp1_Ib = open('three/Ib.txt', 'r').read().split()
Exp1_Ie = open('three/Ie.txt', 'r').read().split()
Exp1_Vb = open('three/Vb.txt', 'r').read().split()
linear_v = []
linear_i = []
beta_vals = []
...
for x in Exp1_Ib:
    ...
    I_cVals3.append((-1*I_eval) - I_bval)
    Exp1_Ib[i] = I_bval
...
U_t3 = (1/(slope))
I_s3 = np.exp(intercept)

i = 0
for x in Exp1_Ib:
    x = float(x)
    beta3 = (I_s3/x)*np.exp(V_bVals[i]/U_t3)
    beta_vals.append(beta3)
    i+=1

beta3 = np.mean(beta_vals)
beta3 = beta_vals
