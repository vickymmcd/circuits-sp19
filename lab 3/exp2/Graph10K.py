...
for x in I_c_10K:
    ...
    V_bval = float(V_b_10K[i])
    gm = x/U_t
    re = U_t/I_eval
    slope = gm/(1+(R/re))
    print(I_eval, V_bval)
    Theo_10K.append(slope* V_bval)
    i = i+1
...
Data1 = plt.plot(V_b_10K, I_c_10K, 'ro', markersize=3, label="R = 10K Ohm")
Data2 = plt.plot(V_b_10K, Theo_10K, 'bo', markersize=3, label="Theoretical Fit")
